...
const sevButton: Record<string, string> = {
  critical: "bg-red-700 hover:bg-red-800 focus:ring-red-200",
  high: "bg-red-700 hover:bg-red-800 focus:ring-red-200",
  medium: "bg-amber-700 hover:bg-amber-800 focus:ring-amber-200",
  low: "bg-slate-800 hover:bg-slate-900 focus:ring-slate-200",
};
...
const fallbackRemediation: Remediation = {
  ...
  console: [
    ...
    "Navigate to IAM",
    "Locate the affected resource and review its configuration",
  ],
            ...
            <div className="space-y-2 px-3 py-2.5">
              <div className="flex flex-wrap gap-1">
                {stmt.actions.map((a) => (
                  <span
                    key={a}
                    className="rounded border border-red-200 bg-red-50 px-1.5 py-0.5 font-mono text-red-700"
                  >
                    {a}
                  </span>
                ))}
      ...
      {scalar.length > 0 && (
        <div className="overflow-hidden rounded-lg border border-zinc-200">
          {scalar.map(([k, v], i) => (
            <div
              key={k}
              className={`flex gap-4 px-4 py-2.5 text-sm ${i % 2 === 0 ? "bg-white" : "bg-zinc-50"}`}
            >
              <span className="w-32 flex-shrink-0 text-zinc-400">{k.replace(/_/g, " ")}</span>
              <span className="break-all font-mono text-zinc-800">{String(v)}</span>
            </div>
          ))}
        </div>
      )}
  ...
  return (
    <div>
      <div className="mb-3 flex items-center justify-between">
        <div className="text-xs font-semibold uppercase tracking-wide text-zinc-500">Suggested Policy</div>
        {!enabled && (
          <button
            onClick={() => setEnabled(true)}
            className="rounded border border-zinc-300 bg-white px-3 py-1 text-xs font-medium text-zinc-700 transition-colors hover:bg-zinc-50 hover:text-zinc-900"
          >
            Generate
          </button>
        )}
      ...
      {enabled && data && data.has_inline_policies && data.cleaned_policies && (
        <div className="space-y-3">
          ...
          <pre className="max-h-72 overflow-y-auto overflow-x-auto whitespace-pre-wrap rounded-lg bg-zinc-950 px-4 py-4 font-mono text-xs leading-relaxed text-zinc-200">
...
export function FindingDrawer({
  ...
  const accent = sevAccent[finding.severity] ?? sevAccent.low;
  const primary = sevButton[finding.severity] ?? sevButton.low;
  ...
  return (
    <>
      ...
      <div className="fixed right-0 top-0 z-50 flex h-full w-full max-w-[560px] flex-col overflow-hidden bg-white shadow-2xl">
        <div className="relative border-b border-zinc-200 bg-white px-7 py-6">
          <div className={`absolute left-0 top-0 h-full w-1 ${accent}`} />
          <div className="flex items-start justify-between gap-4">
            <div className="min-w-0 space-y-3">
              <div className="flex flex-wrap items-center gap-2">
                <span className={`inline-flex items-center rounded-md border px-2.5 py-1 text-xs font-bold uppercase tracking-wide ${headerBadge}`}>
                  {finding.severity}
                </span>
                <span className="font-mono text-xs text-zinc-500">{finding.check_id}</span>
              </div>
          ...
          <div className="mt-5 rounded-xl border border-zinc-200 bg-zinc-50 px-4 py-3">
            <div className="mb-1 text-[11px] font-bold uppercase tracking-[0.18em] text-zinc-400">
              Affected Resource
            </div>
            <p className="break-all font-mono text-sm leading-relaxed text-zinc-800">{finding.resource_arn}</p>
          </div>
        </div>

        <div className="flex-1 space-y-6 overflow-y-auto px-7 py-6">
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
            <div className="rounded-xl border border-zinc-200 bg-white p-4 shadow-sm shadow-zinc-950/[0.03]">
              <div className="mb-2 text-xs font-bold uppercase tracking-[0.16em] text-zinc-400">
                Finding Context
              </div>
              <p className="text-sm leading-7 text-zinc-700">{rem.why}</p>
            </div>
            <div className="rounded-xl border border-zinc-200 bg-white p-4 shadow-sm shadow-zinc-950/[0.03]">
              <div className="mb-2 text-xs font-bold uppercase tracking-[0.16em] text-zinc-400">Risk</div>
              <p className="text-sm leading-7 text-zinc-700">{rem.risk}</p>
            </div>
          </div>
          ...
          {hasEvidence && (
            <div>
              <div className="mb-3 flex items-center justify-between">
                <div className="text-xs font-bold uppercase tracking-[0.14em] text-zinc-500">Scan Details</div>
          ...
          <div>
            <div className="mb-3 flex items-center justify-between">
              <div className="text-xs font-bold uppercase tracking-[0.14em] text-zinc-500">Remediation</div>
              <span className="text-xs text-zinc-400">Choose console steps or AWS CLI</span>
            </div>
            <div className="mb-4 flex w-fit gap-1 rounded-lg bg-zinc-100 p-1">
            ...
            {tab === "console" && (
              <ol className="space-y-3 rounded-xl border border-zinc-200 bg-white p-4 shadow-sm shadow-zinc-950/[0.03]">
                {rem.console.map((step, i) => (
                  <li key={i} className="flex gap-3 text-sm leading-7 text-zinc-700">
                    <span className={`mt-0.5 flex h-6 w-6 flex-shrink-0 items-center justify-center rounded-full text-xs font-bold text-white ${accent}`}>
                      {i + 1}
                    </span>
                    {step}
                  </li>
            ...
            {tab === "cli" && (
              <pre className="overflow-x-auto whitespace-pre-wrap rounded-xl bg-zinc-950 px-4 py-4 font-mono text-xs leading-relaxed text-zinc-200">
                {resolvedCli(finding)}
              </pre>
            )}
          </div>

          <div className="flex items-center gap-4 pb-2 text-xs text-zinc-400">
            ...
            <span>·</span>
            <span>
              Score <span className="font-semibold text-zinc-600">{finding.risk_score}</span>
            </span>
          </div>
        </div>

        <div className="flex gap-2 border-t border-zinc-100 bg-white px-7 py-4">
          <button
            onClick={() => {
              onAction(finding.id, "resolve");
              onClose();
            }}
            className={`flex-1 rounded-lg px-4 py-2.5 text-sm font-semibold text-white transition focus:outline-none focus:ring-4 ${primary}`}
          >
            Resolve
          </button>
          <button
            onClick={() => {
              onAction(finding.id, "snooze");
              onClose();
            }}
            className="flex-1 rounded-lg border border-zinc-300 bg-white px-4 py-2.5 text-sm font-semibold text-zinc-700 transition hover:border-zinc-400 hover:bg-zinc-50"
          >
            Snooze
          </button>
          <button
            onClick={() => {
              onAction(finding.id, "ignore");
              onClose();
            }}
            className="rounded-lg border border-zinc-300 bg-white px-4 py-2.5 text-sm font-semibold text-zinc-700 transition hover:border-zinc-400 hover:bg-zinc-50"
          >
            Ignore
          </button>
        </div>
